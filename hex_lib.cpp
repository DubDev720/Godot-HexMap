...
OffsetCoord qoffset_from_qdoubled(int offset, DoubledCoord h)
{
    int parity = h.col & 1;
    return OffsetCoord(h.col, int((h.row + offset * parity) / 2));
}


DoubledCoord qoffset_to_qdoubled(int offset, OffsetCoord h)
{
    int parity = h.col & 1;
...
OffsetCoord roffset_from_rdoubled(int offset, DoubledCoord h)
{
    int parity = h.row & 1;
    return OffsetCoord(int((h.col + offset * parity) / 2), h.row);
}


DoubledCoord roffset_to_rdoubled(int offset, OffsetCoord h)
{
    int parity = h.row & 1;
...
void test_offset_roundtrip()
{
    ...
    for (int col = -2; col < 3; col++)
    {
        ...
        {
            OffsetCoord offset = OffsetCoord(col, row);
...
void test_offset_to_doubled()
{
    for (int col = -2; col < 3; col++)
    {
        for (int row = -2; row < 3; row++)
        {
            ...
            DoubledCoord qdoubled = DoubledCoord(col * 2 + (row & 1), row);
            ...
            DoubledCoord rdoubled = DoubledCoord(col, row * 2 + (col & 1));
